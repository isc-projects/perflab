	...
	constructor(progname, settings, path) {
		...
		this.checkout = (branch) => {
			...
			if (!repo.vcs || repo.vcs === 'git') {
				...
				return this.spawn('/usr/bin/svn', ['co', `${repo.url}${branch}`, '.'], {cwd: buildPath});
			} else {
				throw Error('unknown repo protocol');
			}
		};

		this.commitlog = async () => {
			let repo = settings.repo;
			if (!repo.vcs || repo.vcs === 'git') {
				let res = await this.spawn('/usr/bin/git', ['log', '-n', 1], {cwd: buildPath, quiet: true});
				return res.stdout;
			} else if (repo.vcs === 'svn') {
				let res = await this.spawn('/usr/bin/svn', ['log', '-l', 1], {cwd: buildPath, quiet: true});
				return res.stdout;
			} else {
				throw Error('unknown repo protocol');
			}
