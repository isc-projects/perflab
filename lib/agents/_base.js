...
class Builder extends Executor {
	...
	constructor(progname, path) {
		...
		this.commitlog = {
			git: () =>
				this.spawn('/usr/bin/git', ['log', '-n', 1], {cwd: buildPath, quiet: true}).then(res => res.stdout),
			svn: () =>
				this.spawn('/usr/bin/svn', ['log', '-l', 1], {cwd: buildPath, quiet: true}).then(res => res.stdout)
		};
