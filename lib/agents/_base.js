...
class Builder extends Executor {
	...
	constructor(progname, settings, path) {
		...
		this.checkout = (branch) => {
			...
			if (repo.git) {
				return this.spawn('/usr/bin/git', ['clone', '--depth', 1, '-b', branch, repo.git, '.'], {cwd: buildPath});
			} else if (repo.svn) {
				this.spawn('/usr/bin/svn', ['co', `${repo.svn}${branch}`, '.'], {cwd: buildPath})
			} else {
