...
module.exports = class DHCPDAgent extends Agents.Builder {
	...
	constructor(settings, config) {
		...
		let repo = settings.repo.dhcpd;
		let testPath = path + '/tests/' + config._id;
		let buildPath = testPath + '/build';
		let runPath = testPath + '/run';
		let etcPath = runPath + '/etc';

		super('dhcpd', testPath);

		let cmd = settings.command.dhcpd || './sbin/dhcpd';
		let wrapper = (config.wrapper && config.wrapper.length) ? config.wrapper : settings.wrapper.dhcpd;
		...
		// gets the required BIND libraries
		this.target('getbind', 'checkout', () => this.spawn('/bin/sh',
			['./util/bind.sh', `--remote=${settings.repo.bind}`, 'HEAD'],
		...
		let getInfo = () => Promise.join(this.commitlog(repo), getVersion(),
			(commitlog, version) => commitlog + '\n' + version
		);

		// starts the server under test
		let start = () => this.daemon(cmd, [
			settings.args.dhcpd, '-cf', 'etc/dhcpd.conf', config.args.server
