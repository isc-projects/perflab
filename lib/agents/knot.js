...
module.exports = class KnotAgent extends Agents.Builder {
	...
	constructor(settings, config) {
		...
		this.target('checkout', 'prepare', () =>
			this.checkout.git(settings.repo.knot, config.branch));
		...
		let getVersion = () => this.spawn('./sbin/knotd', '-V', {cwd: runPath, quiet: true});
		...
		let start = () => this.daemon(cmd, [
			settings.args.knot, '-v', config.args.knot
		], {cwd: runPath}, / server started /m);
