...
//
module.exports = class KnotAgent extends Agents.Builder {
	...
	constructor(settings, config) {
		...
		super('Knot2', testPath);

		let cmd = settings.command.knot || './sbin/knotd';
		let wrapper = (config.wrapper && config.wrapper.length) ? config.wrapper : settings.wrapper.knot;
		...
		let start = () => this.daemon(cmd, [
			settings.args.knot, '-v', config.args.server
