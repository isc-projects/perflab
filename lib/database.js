...
class Database {
	constructor (settings) {
		...
		// if repeat gets disabled then the job gets disabled too
		...
		this.takeNextFromQueue = () =>
			query((db) => db.collection('config')
					.findOneAndUpdate(
						{'queue.running': {$ne: true}, 'queue.enabled': true, 'deleted': {$ne: true}},
						{$set: {
							'queue.running': true,
		...
		this.updateStatsByRunId = (run_id) =>
			query((db) => db.collection('test')
				.mapReduce(test_stats_map, test_stats_reduce, {
					...
					{_id: oid(run_id)},
					{$set: { stats: mr.results.length ? mr.results[0].value : {count: 1}}}
				)));
