...
class Database {
	...
	constructor (settings) {
		...
		this.init = async () => {
			...
			this.setQueueEntryEnabled = async (id, obj) => {
				var set = {
			...
			this.setQueueEntryRepeat = async (id, obj) => {
				let set = { 'queue.repeat': !!obj.repeat };
			...
			this.setQueueEntryPriority = async (id, obj) =>
				db.collection('config').updateOne({_id: oid(id)}, {$set: {'queue.priority': obj.priority}});
			...
			this.getConfigById = async (id) =>
				db.collection('config').findOne({_id: oid(id)});

			// deletes the specified configuration (not really)
			this.deleteConfigById = async (id) =>
				db.collection('config').updateOne({_id: oid(id)}, {$set: {deleted: true}});
			...
			this.getMemoryStatsByRunId = async (run_id, mem) =>
			...
			this.getRunsByConfigId = async (config_id, skip, limit) => {
				skip = skip || 0;
				limit = limit || 0;
			...
			this.getControl = async (obj) => db.collection('control').findOne();

			// set the global paused status
			// 'obj' must contain {"paused": <boolean>}
			this.setPaused = async (obj) =>
