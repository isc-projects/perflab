...
class Database {
	...
	constructor (settings) {
		...
		this.init = async () => {
			...
			this.deleteConfig = async (id, {}, {}) => Promise.all([
				db.collection('config').delete({_id: oid(id)}),
				db.collection('run').delete({config_id: oid(id)}),
				db.collection('test').delete({config_id: oid(id)})
			]);
			...
			this.getConfigListAll = async ({}, {archived}) => {
				let filter = !!archived ? {} : {archived: {$ne: true}};
				return db.collection('config')
						 .find(filter,  listView)
						 .toArray();
			};
