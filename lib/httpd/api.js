...
function getAgent(agents, field) {
	if (field in agents) {
		return Promise.resolve(agents[field].configuration);
	} else {
		return Promise.resolve({});
	}
}

function getAgents(agents, protocol) {
	let results = [];
	...
	if (protocol !== undefined) {
		results = results.filter(c => c.protocol === protocol);
	}
...
//		'DELETE /:id':					handler(db.deleteConfigById),
	...
	'/agent/server': {
		...
		'GET /_protocol/:proto':		handler(getAgents.bind(this, agents.servers)),
		'GET /:name':					handler(getAgent.bind(this, agents.servers))
	},
	'/agent/client': {
		'GET /':						handler(getAgents.bind(this, agents.clients)),
		'GET /_protocol/:proto':		handler(getAgents.bind(this, agents.clients)),
